...
const perfRegexes = require('perf-regexes');
 ...
 */
const safeRegex = (re, flags) => new RegExp(re.source.replace(/@LE/g, perfRegexes.EOL.source), flags);
...
const skipQuotes = function (ctx, index) {
    const buffer = ctx.buff;
    const re = buffer[index] === '"' ? perfRegexes.JS_DQSTR : perfRegexes.JS_SQSTR;
    re.lastIndex = index;
    if (!re.exec(buffer)) {
...
const normalize = (ctx, start, end) => {
    ...
    const str = ctx.buff.slice(start, end).replace(perfRegexes.EOL, ctx.eolChar);
...
/** Matches comments */
const reCmnt = {
    '*': perfRegexes.JS_MLCMNT,
    '/': perfRegexes.JS_SLCMNT,
};
/**
 * Comment handler.
 *
